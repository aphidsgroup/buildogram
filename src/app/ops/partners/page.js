      ...
      {filtered.length > 0 ? (
        <div className="grid-3" style={{ gap: '20px', marginBottom: '40px' }}>
          {filtered.map(p => {
            ...
            return (
              <div key={p.id} className="card" style={{ position: 'relative', padding: '24px', cursor: 'pointer', transition: 'all 0.2s', border: selected?.id === p.id ? '2px solid #FFDA01' : '1px solid var(--border)' }}
