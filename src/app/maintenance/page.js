  ...
  return (
    <>
      ...
      <section style={{ background: 'var(--secondary)', color: 'white', padding: '100px 0 80px', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 30% 60%, rgba(255,218,1,0.06) 0%, transparent 55%)' }} />
        <div className="container" style={{ position: 'relative', textAlign: 'center' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'rgba(255,218,1,0.12)', border: '1px solid rgba(255,218,1,0.2)', borderRadius: '999px', padding: '6px 18px', marginBottom: '24px' }}>
            <span style={{ color: '#FFDA01', fontSize: '12px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' }}>Home Maintenance</span>
          </div>
          <h1 style={{ color: 'white', fontSize: 'clamp(34px, 5vw, 60px)', lineHeight: 1.1, marginBottom: '24px' }}>
            Maintain your property<br />
            <span style={{ color: '#FFDA01' }}>with proof of every fix.</span>
          </h1>
          ...
          <div className="grid-3">
            {amcPlans.map(p => (
              <div key={p.plan} style={{
                ...
                border: p.highlight ? '2px solid #FFDA01' : '1px solid var(--border)',
                background: p.highlight ? '#292929' : 'white',
                position: 'relative',
              }}>
                {p.highlight && (
                  <div style={{ position: 'absolute', top: '-14px', left: '50%', transform: 'translateX(-50%)', background: '#FFDA01', color: '#292929', borderRadius: '999px', padding: '4px 16px', fontSize: '12px', fontWeight: 700, whiteSpace: 'nowrap' }}>Most Popular</div>
                )}
                <h3 style={{ fontSize: '20px', marginBottom: '8px', color: p.highlight ? 'white' : '#292929' }}>{p.plan}</h3>
                <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontSize: '28px', fontWeight: 800, color: p.highlight ? '#FFDA01' : '#292929', marginBottom: '20px' }}>{p.price}</div>
                <ul style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '28px' }}>
                  {p.features.map(f => (
                    <li key={f} style={{ display: 'flex', alignItems: 'flex-start', gap: '10px', fontSize: '14px', color: p.highlight ? 'rgba(255,255,255,0.8)' : 'var(--text)' }}>
                      <span style={{ background: p.highlight ? '#FFDA01' : '#292929', color: p.highlight ? '#292929' : '#FFDA01', borderRadius: '50%', width: '18px', height: '18px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: '10px', fontWeight: 800, flexShrink: 0 }}>✓</span>
                      {f}
                    </li>
                  ))}
                </ul>
