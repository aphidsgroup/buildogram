...
export default function ContextualWhatsAppWidget({ context }) {
  ...
  useEffect(() => {
    ...
    const fab = btnRef.current;
    const showHint = () => fab.classList.add('is-hinted');
    const hideHint = () => fab.classList.remove('is-hinted');
  ...
  return (
    <a
      ...
      className={`bg-wa-fab${mounted ? ' is-mounted' : ''}`}
      onClick={handleClick}
    >
      <span aria-hidden="true" className="bg-wa-fab__glow" />
      {/* WhatsApp inline SVG */}
      <svg
        width="28" height="28"
        viewBox="0 0 24 24"
      ...
      <span className="bg-wa-fab__label-wrap">
        <span className="bg-wa-fab__label">Chat with us</span>
      </span>
    </a>
  );
}
