...
export function computeExcelBoq(inputs, options = { mode: 'excel', rateMap: null, marginPct: 0 }) {
  ...
  // 1. Foundation & Column
  let qEarthExcav = 0; console.log('excD mapping check:');
  let qPCC = 0;
  let qFootingConc = 0;
  let qColumnConc = 0;
  for (const f of foundation) {
    ...
    qEarthExcav += nos * fL * fB * excD; console.log(nos, fL, fB, excD);
    qPCC += nos * fL * fB * pccD;
