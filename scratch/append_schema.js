...
function addRelation(modelName, relationLine) {
  const modelRegex = new RegExp('(model \\b' + modelName + '\\b \\{[^}]*)\\}', 'g');
  schema = schema.replace(modelRegex, (match, p1) => {
    if (!p1.includes(relationLine.split(' ')[0])) {
      return p1 + '  ' + relationLine + '\n}\n';
    }
    return match;
  });
}

addRelation('leads', 'property_passports property_passports[]');
addRelation('partners', 'property_passports property_passports[]');
addRelation('case_studies', 'property_passports property_passports[]');
addRelation('material_quote_requests', 'property_passports property_passports[]');
addRelation('users', 'property_passport_records property_passport_records[]');
