...
- **1 条/天**（硬规则，不可超发）
- 发布时间：**固定早 8 点（北京时间）**——账号实测最优窗口：固定早 8 点后爆款命中率从 5% → 15%（见第九节关键转折点）。17 点 / 23 点为二级窗口，只在早 8 点档期冲突时启用
...
### 时间窗口参考（基于 3,861 条数据分析——大盘统计，仅作二级参考）

> ⚠️ **结论以自己账号实测为准**：@WeiYipei 实测最优 = **早 8 点**（固定后爆款命中率 5% → 15%）。下表是外部 3,861 条推文的大盘统计，17/23 点作为二级窗口备选——大盘最优 ≠ 你的账号最优，跑 2-4 周 tweet-log 后按数据锁定自己的窗口。
...
大盘月内排名：17 点 > 23 点 > 13 点 > 11 点 > 20 点（二级窗口取 17 / 23 点）
...
写完 ≠ 发完。完整闭环是：**写 → 审 → 发 → 24h 回收 → 周报**。

### 发布（人工审核后走浏览器）

1. Agent 按排期写稿，过三次翻译 + 五件套检查（上一节）
2. **人工审核**：稿子先给账号主人过目——agent 不得跳过这步直接发布
3. 审核通过后用 Chrome MCP（`mcp__claude-in-chrome__*` 工具）在已登录的浏览器里发布：导航到 x.com → 填入正文 → 发布 → 首条 reply 挂 CTA 链接（红线 3：链接不进正文）
4. 发布成功后立即把 Tweet ID + 发布时间记入 tweet-log

### 24h 数据回收

发布满 24h 后（次日同时段）：

1. 用 Chrome MCP 打开该推文的 analytics（或推文详情页）读取展示量 / 互动数据
2. 回填 tweet-log 对应行的「展示量 / 互动」列——**不回填的 log 等于没记**
3. 展示量 < 300 → 对照死亡开头黑名单和三次翻译表定位原因，记进备注列

### 周报循环

每周固定一天（建议周日）：汇总本周 tweet-log → 出周报（模板见第七节）→ 调整下周内容类型权重与排期。数据回收是周报的输入——跳过回收，周报就是空转。
