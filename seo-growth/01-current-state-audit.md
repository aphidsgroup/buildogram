...
> **READINESS PASS 3 UPDATE (2026-07-25):** A further claim class not identified in the original audit was remediated before deployment — **categorical locality soil/flood/foundation statements and locality construction rates** across 28 area records, 26 legacy localities, the locality page renderers, LocalBusiness schema and the `/construction-in-chennai` hub. These were safety-relevant (construction users may act on them) and carried no cited source. Also corrected in this pass: 51 further "verified contractor/supplier network" claims hidden in data files, the locality generator template, directory UI, a notification template and brand positioning; a NAP defect emitting placeholder phone/postcode in locality JSON-LD; and undated material/works rate strips. See `19-…md` §15 for the full scan table.
